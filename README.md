...
нестандартный порт, fail2ban с вечным баном, автообновления, бэкапы,
...
```bash
# 0) залей файлы на сервер и сделай исполняемыми
chmod +x 00-init.sh 01-init.sh 02-init.sh
...
- 💾 Бэкап старых конфигов в `/root/ssh-hardening-backup-*`, проверка через `sshd -t`, применение через корректную работу с `ssh.socket` (актуально для 24.04).
...
- 🗂️ Бэкап конфигов: `etckeeper` (git в `/etc`) + ежедневный архив `/etc` в `/var/backups/config` (ротация).
...
> whitelist своего IP — самая важная защита от самоблокировки. Скрипт
> кладёт твои адреса в `/etc/fail2ban/ignoreip.local` — этот файл повторные
> запуски **не перезатирают**.
...
```bash
# SSH слушает новый порт и сессия по ключу работает
...
# таймеры бэкапа/аудита/автообновлений
systemctl list-timers --all | grep -E 'config-backup|lynis|offsite'
...
- Ubuntu 22.04 / 24.04 (Debian-совместимо в основном), `systemd`.
- Доступ `root` (через `sudo`).
- Доступ в интернет (для `apt`, lynis, отгрузки бэкапов).
