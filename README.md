...
| `backup-offsite.sh` | *(опционально)* Зашифрованная отгрузка бэкапов на внешнее FTP-хранилище | настройка через конфиг |

**Порядок запуска:** `00 → 01 → 02`. `backup-offsite.sh` — когда захочешь off-site копии.
...
### `backup-offsite.sh` — *(опционально)* off-site бэкап

- Отгрузка архива на **внешнее FTP-хранилище** (например, услуга SpaceCore) через `lftp`.
- 🔐 **Клиентское шифрование GPG AES-256** перед отправкой (FTP идёт открытым текстом, хранилище стороннее).
- ⏱️ Ежедневный systemd-таймер, ротация копий локально и на хранилище.
- Реквизиты вписываются в `/etc/backup-offsite.conf` (права `600`).
